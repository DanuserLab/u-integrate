function varargout = ActivityMapDescriptionForGCAProcessMLGUI(varargin)
%ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI MATLAB code file for ActivityMapDescriptionForGCAProcessMLGUI.fig
%      ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI, by itself, creates a new ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI or raises the existing
%      singleton*.
%
%      H = ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI returns the handle to a new ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI or the handle to
%      the existing singleton*.
%
%      ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI('Property','Value',...) creates a new ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to ActivityMapDescriptionForGCAProcessMLGUI_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI('CALLBACK') and ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI('CALLBACK',hObject,...) call the
%      local function named CALLBACK in ACTIVITYMAPDESCRIPTIONFORGCAPROCESSMLGUI.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help ActivityMapDescriptionForGCAProcessMLGUI

% Last Modified by GUIDE v2.5 19-Jul-2021 13:57:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ActivityMapDescriptionForGCAProcessMLGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @ActivityMapDescriptionForGCAProcessMLGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before ActivityMapDescriptionForGCAProcessMLGUI is made visible.
function ActivityMapDescriptionForGCAProcessMLGUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
if isequal(funParams.adf, true)
  set(handles.checkbox_adf, 'Value', 1)
else
  set(handles.checkbox_adf, 'Value', 0)
end
...
% UIWAIT makes ActivityMapDescriptionForGCAProcessMLGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ActivityMapDescriptionForGCAProcessMLGUI_OutputFcn(hObject, eventdata, handles)
...
if get(handles.checkbox_adf, 'Value') == 1
    funParams.adf = true;
else
    funParams.adf = false;
end
...
% --- Executes on button press in checkbox_adf.
function checkbox_adf_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_adf (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_adf
