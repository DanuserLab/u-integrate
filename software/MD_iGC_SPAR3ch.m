function MD_iGC_SPAR3ch(MD, iChan1, iChan2, iChan3, chan1Name, chan2Name,chan3Name, layerMax, ...
    figuresDir, twlagMax, twlagMaxReg, varargin)
% MD_iGC_SPAR3ch() COMPUTE P-values for instantaneous Granger-Causality (iGC) 
% from Chan1 to Chan2 given Chan3 in each window. 
%
% Regression Model:%
%   iChan2_t ~ lagged iChan2_t + lagged iChan1_t + lagged iChan3_t
%   (AR+Reg+control)
%
% Updates:
...
% J Noh, 20201/02/05. Rename and add comments.
% J Noh,2020/06/03. Add .fig output for '_PvalPairedMaps_Vslztn' plot.
...
fsaveName0 = ['frCh', num2str(iChan1), 'toCh', num2str(iChan2), 'givenCh', num2str(iChan3)];
disp(fsaveName0)
% 2021/02/11
fsaveName1 = ['fr_', chan1Name, '_to_', chan2Name, '_given_', chan3Name];
disp(fsaveName1)
...
if layerMax > 1
    ...
    if numel(imActmap3) == 1
        for indL = 2:layerMax; imActmap3{indL} = imActmap3{1}; end
    end
end
...
for indL = 1:layerMax
    ...
    % window-wise regression
    %winch2Pvec{indL} = nan(wmax, wlagMax+1);
    %winch1Pvec{indL} = nan(wmax, wlagMax+1);
    %tic
    
    %ch2Pvectmp = nan(wmax, 2*wlagMax+1+2 +2*wlagMax+1+2 + 2*wlagMaxReg+2);
    % for arCtlmaxLag (P0 P1L P1R U D; cP0 cP1L cP1R U D) due to Lso
    %ch1Pvectmp = nan(wmax, 1);               % for regLag (Q0, Q1)
    %tmpIndVarsMat = nan(wmax, (1+2*wlagMax+2)*tlagMax + (1+2*wlagMax+2)*(tlagMax+1) ...
    %    + (1+2*wlagMaxReg+2)*(tlagMaxReg+1));
    %tmpmaxLagsMat = nan(wmax, (2*wlagMax+1));
    %tmpCoefMat = nan(wmax, (1+2*wlagMax+2)*tlagMax*2 + (1+2*wlagMaxReg+2)*tlagMaxReg);
    %tmpCoefMat = tmpIndVarsMat;
    ...
    [arMdlAvBICmap, arMdlAvBICcurve, reducedMdlAvBICmap, reducedMdlAvBICcurve, ...
        ...
        Pw0, Pw1, Qw0, modelIndVars_mat, modelCoef_mat, winFits_mat, resi_mat, yhatF_mat, yhatR_mat] = ...
    SPAR_3ch_mdlSlctn_iGC_3steps(zmapx, zmapy, zmapz, tlagMax, wlagMax, ...
    ...
    title([num2str(indL), 'L ', 'Ctrl Mdl: Pw0 Pw1= ', num2str(Pw0), ', ', num2str(Pw1)])
    xlabel('Model orders')
    ylabel('Avg IC')    
