function samples = sampleStackWindowsCounts(windows,stack,mask)
%SAMPLEIMAGEWINDOWS samples the stack intensity in each of the input windows
% Modified for counts output by Jungsik Noh
% 2016/06/07
%
...
sampledFields = {'avg','med','std','min','max','n', 'effectiveN', 'sum','nWinMask'};
...
for iSlice = 1:nSlices    
    for iBand = 1:nBands(iSlice)
       ...
       if ~isempty(windows{iSlice}{iBand})
            ...
            %necessary.
            winMask1 = winMask; % & mask;    Changed!
            %mask(winMask1) = false; %Remove these pixels from the mask. If there's something wrong with the propagation method, we don't want ot over-sample pixels.

            if nnz(winMask1)>0            
                for nz=1:size(stack,3)
                    im=stack(:,:,nz);
                    %Sample the image with this mask
                    currSamp = im(winMask1(:));
                    ...
                    samples(nz).effectiveN(iSlice,iBand) = sum(~isnan(currSamp));
                    
                    samples(nz).sum(iSlice,iBand) = sum(currSamp, 'omitnan');
                    samples(nz).nWinMask(iSlice,iBand) = sum(winMask(:)==1);
                end
            end            
       end
