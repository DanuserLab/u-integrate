function varargout = QuiescentWindowDetectionProcessMLGUI(varargin)
%QUIESCENTWINDOWDETECTIONPROCESSMLGUI MATLAB code file for QUIESCENTWINDOWDETECTIONProcessMLGUI.fig
%      QUIESCENTWINDOWDETECTIONPROCESSMLGUI, by itself, creates a new QUIESCENTWINDOWDETECTIONPROCESSMLGUI or raises the existing
%      singleton*.
%
%      H = QUIESCENTWINDOWDETECTIONPROCESSMLGUI returns the handle to a new QUIESCENTWINDOWDETECTIONPROCESSMLGUI or the handle to
%      the existing singleton*.
%
%      QUIESCENTWINDOWDETECTIONPROCESSMLGUI('Property','Value',...) creates a new QUIESCENTWINDOWDETECTIONPROCESSMLGUI using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to QUIESCENTWINDOWDETECTIONProcessMLGUI_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      QUIESCENTWINDOWDETECTIONPROCESSMLGUI('CALLBACK') and QUIESCENTWINDOWDETECTIONPROCESSMLGUI('CALLBACK',hObject,...) call the
%      local function named CALLBACK in QUIESCENTWINDOWDETECTIONPROCESSMLGUI.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help QUIESCENTWINDOWDETECTIONProcessMLGUI
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @QuiescentWindowDetectionProcessMLGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @QuiescentWindowDetectionProcessMLGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before QuiescentWindowDetectionProcessMLGUI is made visible.
function QuiescentWindowDetectionProcessMLGUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
set(handles.edit_ratioThreshold, 'String',num2str(funParams.ratioThreshold))

% Choose default command line output for QuiescentWindowDetectionProcessMLGUI
...
function varargout = QuiescentWindowDetectionProcessMLGUI_OutputFcn(hObject, eventdata, handles)
...
if isnan(str2double(get(handles.edit_ratioThreshold, 'String'))) ...
    || str2double(get(handles.edit_ratioThreshold, 'String')) < 0 ...
    || str2double(get(handles.edit_ratioThreshold, 'String')) > 1
  errordlg('Please provide a valid input for ''Threshold for percentage of quiescent windows''.','Setting Error','modal');
  return;
end
...
funParams.ratioThreshold = str2double(get(handles.edit_ratioThreshold, 'String'));
...
function edit_ratioThreshold_Callback(hObject, eventdata, handles)
% hObject    handle to edit_ratioThreshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_ratioThreshold as text
%        str2double(get(hObject,'String')) returns contents of edit_ratioThreshold as a double


% --- Executes during object creation, after setting all properties.
function edit_ratioThreshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_ratioThreshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
