...
for i=1:numel(ML.movieDataFile_) % QZ there are 2 movieData.mat files, each has 2 channels, but only channel 'Actin' used in this pipeline.
...
MLsummary_XcorrCurvesVelAcf(ML,1,0,'Actin','Vel', maxLayer, 'mapDescriptivesF', 'mapCrossCorrF', ...
    'lagMax0', 40, 'outDirName', 'Xcf_ch1ch0_F_lagMax40')
...
mapDescriptives_OneChan(MD, iChan0, 1, chan0Name, chan0Title, figuresDir, ...
     'adf', 1, 'omittedWindows', [], 'subFrames', subFr, 'numPerm', 100, 'topograph', 'on', 'parpoolNum', 2) %QZ I changed 'parpoolNum' from 6 to 2.
...
mapDescriptives_OneChan(MD, iChan, maxLayer, chanName, chanTitle, figuresDir, ...
     'adf', 1, 'omittedWindows', [], 'subFrames', subFr, 'numPerm', 100, 'topograph', 'on', 'parpoolNum', 2, 'WithN', 0) %QZ I changed 'parpoolNum' from 6 to 2.
...
layerMax = maxLayer;
%
mapXcorrCurvePermutation_Vel(MD, iChan, chanName, layerMax, figuresDir, ...
    'impute', 0, 'subFrames', subFr, 'parpoolNum', 2, 'WithN', 0, 'numPerm', 100, 'lagMax', 40) %QZ I changed 'parpoolNum' from 6 to 2.
...
[protMask1, retMask1] = phaseMasking(MD, smParamTh, figuresDir, ...
                    'minimumRunLength', 5, 'subFrames', subFr); % QZ MD was loaded to workspace.
...
phaseDescriptives_MaxMinVel_OneChan(MD, iChan, maxLayer, chanName, chanTitle, smParamTh, samplingBw, figuresDir, ...
        'WithN', 0, 'impute', 1, 'omittedWindows', omitWin, 'subFrames', subFr, 'movingAvgSmoothing', 0)
