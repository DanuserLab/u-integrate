%% example_pipeline_DX_FPAEME_1chan_biosensor.m
...
% movieList, MDindex, maxLayer, chname, index chan, mapDesc outDirName, XCorr outDirName
    ML_DX_1chan(ML, i, maxLayer, 'GEFH1', 1, 'mapDescriptivesF', 'mapCrossCorrF')
end
...
for i=1:numel(ML.movieDataFile_)
    ...
    ML_phaseDescriptives(ML, i, maxLayer, 'GEFH1', 1, 'phaseDescriptives0p4mRL5')
...
MLsummary_FluctuationProfiling(ML, 'GEFH1', maxLayer, 'phaseDescriptives0p4mRL5', ...
...
tic
for i=1:numel(ML.movieDataFile_)
    ML_meanCV_alongDepth(ML, i, 7, 'GEFH1', 1, 'mapDescriptives_alongDepth')
end
toc
...
MLsummary_alongDepth(ML,1,7,'GEFH1','mapDescriptives_alongDepth', ...
...
mapDescriptives_OneChan(MD, iChan, maxLayer, chanName, chanTitle, figuresDir, 'adf', 1, ...
     'omittedWindows', [], 'subFrames', subFr, 'numPerm', 100, 'topograph', 'on', 'parpoolNum', 2, 'WithN', 1) %QZ I changed 'parpoolNum' from 6 to 2.
...
mapXcorrCurvePermutation_Vel(MD, iChan, chanName, layerMax, figuresDir, 'impute', 0, ...
    'subFrames', subFr, 'parpoolNum', 2, 'WithN', 1, 'numPerm', 100, 'lagMax', 40) %QZ I changed 'parpoolNum' from 6 to 2.
