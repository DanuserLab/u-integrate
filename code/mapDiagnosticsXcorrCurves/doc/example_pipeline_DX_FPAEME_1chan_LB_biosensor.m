%% example_pipeline_DX_FPAEME_1chan_LB_biosensor.m
%% The only input for this pipeline is the movieList object loaded to workspace.
% Jungsik Noh, 2018/05/07

%%  sampleMovieWindowsWithN to not include 0 values for the 1st layer

ML.getMovies()

for i=1:numel(ML.movieDataFile_)
    sampleMovieWindowsWithN(ML.movies_{i});
end
...
% movieList, MDindex, maxLayer, chname, index chan, mapDesc outDirName, XCorr outDirName    
    ML_DX_1chan_LB(ML, i, maxLayer, 'GEFH1', 1, 'mapDescriptives_LB', 'mapCrossCorr_LB')
end
...
MLsummary_XcorrCurvesVelAcf(ML,1,0,'GEFH1','Vel', maxLayer, ...
...
for i=1:numel(ML.movieDataFile_)
    ...
    ML_phaseDescriptives_LB(ML, i, maxLayer, 'GEFH1', 1, 'phaseDescriptives0p4mRL5_LB')
end
toc
...
MLsummary_FluctuationProfiling(ML, 'GEFH1', maxLayer, 'phaseDescriptives0p4mRL5_LB', ...
...
tic
for i=1:numel(ML.movieDataFile_)
    ML_meanCV_alongDepth_LB(ML, i, 7, 'GEFH1', 1, 'mapDescriptives_alongDepth_LB')
end
toc
...
MLsummary_alongDepth(ML,1,7,'GEFH1','mapDescriptives_alongDepth_LB', ...
...
mapDescriptives_OneChan(MD, iChan, maxLayer, chanName, chanTitle, figuresDir, 'adf', 1, ...
     'omittedWindows', [], 'subFrames', subFr, 'numPerm', 100, 'topograph', 'on', 'parpoolNum', 6, 'WithN', 1)
...
mapXcorrCurvePermutation_Vel(MD, iChan, chanName, layerMax, figuresDir, 'impute', 0, ...
    'subFrames', subFr, 'parpoolNum', 6, 'WithN', 1, 'numPerm', 100, 'lagMax', 40)
...
mapDescriptives_OneChan(MD, iChan, maxLayer, chanName, chanTitle, figuresDir, 'adf', 1, ...
     'movingAvgSmoothing',0, 'omittedWindows', omitWin, 'subFrames', subFr,'numPerm', 100, 'topograph', 'on', 'WithN', 1)
...
phaseDescriptives_OneChan(MD, iChan, maxLayer, chanName, chanTitle, Mask0, samplingBw, figuresDir, ...
        'WithN', 1, 'impute', 1, 'omittedWindows', omitWin, 'subFrames', subFr, 'movingAvgSmoothing', 0)
...
mapDescriptives_meanCV_alongDepth(MD, iChan, maxLayer, chanName, figuresDir, ...
    'impute',0,'WithN',1, 'subFrames', subFr, 'omittedWindows', [])
...
mapDescriptives_meanCV_alongDepth(MD, iChan,maxLayer, chanName,figuresDir, ...
    'impute',0,'WithN', 1, 'subFrames', subFr, 'omittedWindows', omitWin)
