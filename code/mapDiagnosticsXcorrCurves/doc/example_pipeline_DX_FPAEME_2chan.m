...
% movieList, MDindex, maxLayer, chname, index chan, mapDesc outDirName, XCorr outDirName
    ML_DX_1chan(ML, i, maxLayer, 'Arp3', 1, 'mapDescriptivesF', 'mapCrossCorrF')
    ML_DX_1chan(ML, i, maxLayer, 'mDia1', 2, 'mapDescriptivesF', 'mapCrossCorrF')
    ML_DX_xcorr2chan(ML, i, maxLayer, 'mDia1', 'Arp3', 2, 1, 'mapCrossCorrF')
end
...
MLsummary_XcorrCurvesVelAcf(ML,1,0,'Arp3','Vel', maxLayer, ...
    'mapDescriptivesF', 'mapCrossCorrF', 'lagMax0', 50, ...
    'outDirName', 'Xcf_ch1ch0_F_lagMax50')
%
maxLayer = 2
...
for i=1:numel(ML.movieDataFile_)
    ...
    ML_phaseDescriptives(ML, i, maxLayer, 'Arp3', 1, 'phaseDescriptives0p5mRL5')
end
toc
...
MLsummary_FluctuationProfiling(ML, 'Arp3', maxLayer, 'phaseDescriptives0p5mRL5', ...
...
for i=1:numel(ML.movieDataFile_)
    % check smParam and minimumRunLength within ML_ function.
    ML_phaseDescriptives(ML, i, maxLayer, 'mDia1', 2, 'phaseDescriptives0p5mRL5')
end
toc
%  FPAME summary at the ML level
...
tic
for i=1:numel(ML.movieDataFile_)
    ML_meanCV_alongDepth(ML, i, 5, 'Apr3', 1, 'mapDescriptives_alongDepth')
end
toc

%  MLsummary_alongDepth

MLsummary_alongDepth(ML,1,5,'Arp3','mapDescriptives_alongDepth', ...
