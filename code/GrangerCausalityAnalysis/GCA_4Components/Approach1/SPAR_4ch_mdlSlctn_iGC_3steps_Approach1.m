function [arMdlAvBICmap, arMdlAvBICcurve, reducedMdlAvBICmap, reducedMdlAvBICcurve, ...
    ...
    SPAR_4ch_mdlSlctn_iGC_3steps_Approach1(Mapsx, Mapsy, Mapsz, Mapsv, ...
...
% include V_t(w,l) and all adjacent signals at t
      vLag=cell(2*wlagMax+1+2, 1);
         X1 = [X1, vadj{1}];
        vLag{1} = myLagmatrix(vadj{1}, Pw0);
        X1 = [X1, vLag{1}];
        if wlagMax > 0
            for k = 1:1
                X1 = [X1, vadj{2}];
                vLag{2*k} = myLagmatrix(vadj{2*k}, Pw1);
                X1 = [X1, vLag{2}];
                X1 = [X1, vadj{3}];
                vLag{2*k+1} = myLagmatrix(vadj{2*k+1}, Pw1);
                X1 = [X1, vLag{2*k+1}];

            end
        end
        vLag{1+2*wlagMax+1} = myLagmatrix(vadj{1+2*wlagMax+1}, Pw1);
        X1 = [X1, vadj{4}, vLag{4}];
        vLag{1+2*wlagMax+2} = myLagmatrix(vadj{1+2*wlagMax+2}, Pw1);
        X1 = [X1, vadj{5}, vLag{5}];

         %  mapx's l/r/u/d now go into control variables.
        xcLag = cell(2*wlagMaxReg+1+2, 1);
        if wlagMaxReg > 0
            for k = 1:1
                xcLag{2*k} = myLagmatrix(xadj{2*k}, Pw1);
...
tmpIndvec3 = [ones(1,Pw0+1), zeros(1,tlagMax-Pw0), ...
repmat([ones(1,Pw1+1),zeros(1,tlagMax-Pw1)],1,4)];
tmpIndvec4= repmat([ones(1,Pw1+1),zeros(1,tlagMax-Pw1)],1,4);  
