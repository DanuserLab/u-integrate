function [arMdlAvBICmap, arMdlAvBICcurve, reducedMdlAvBICmap, reducedMdlAvBICcurve, ...
    ...
    SPAR_4ch_mdlSlctn_iGC_3steps_Approach2(Mapsx, Mapsy, Mapsz, Mapsv, ...
    tlagMax, wlagMax, tlagMaxReg, wlagMaxReg, regPvecMat, ICtype)
% SPAR_4ch_mdlSlctn_iGC_3steps() IMPLEMENT the regression fitting and model
...
for w = (1+w00):(wmax-w00)    
    ...
    for j = 1:size(regPvecMat, 1)
        ...
        if any(any(isnan([y, X])))
            ...
            bhat = X\y; sse = (y-X*bhat)'*(y-X*bhat);  
...
for w = (1+w00):(wmax-w00)
    ...
    for j1 = 1:nJ1
        Pw01c = regPvecMat(j1,1);
        Pw11c = regPvecMat(j1,2);

        for j2 = 1:nJ2
            Pw02c = regPvecMat(j2,1);
            Pw12c = regPvecMat(j2,2);
            ...
            for j3 = 1:nJ3
                ...
                X1 = [X1, zadj{1}];
                zLag{1} = myLagmatrix(zadj{1}, Pw01c);
                X1 = [X1, zLag{1}];

                if wlagMax > 0
                    for k = 1:1
                        X1 = [X1, zadj{2}];
                        zLag{2*k} = myLagmatrix(zadj{2*k}, Pw11c);
                        X1 = [X1, zLag{2}];
                        X1 = [X1, zadj{3}];
                        zLag{2*k+1} = myLagmatrix(zadj{2*k+1}, Pw11c);
                        X1 = [X1, zLag{2*k+1}];
                    end
                end

                zLag{1+2*wlagMax+1} = myLagmatrix(zadj{1+2*wlagMax+1}, Pw11c);
                X1 = [X1, zadj{4}, zLag{4}];
                zLag{1+2*wlagMax+2} = myLagmatrix(zadj{1+2*wlagMax+2}, Pw11c);
                X1 = [X1, zadj{5}, zLag{5}];

                %% Include V_t(w,l)
                vLag = cell(2*wlagMax+1+2, 1);
                X1 = [X1, vadj{1}];
                vLag{1} = myLagmatrix(vadj{1}, Pw02c);
                X1 = [X1, vLag{1}];

                if wlagMax > 0
                    for k = 1:1
                        X1 = [X1, vadj{2}];
                        vLag{2*k} = myLagmatrix(vadj{2*k}, Pw12c);
                        X1 = [X1, vLag{2}];
                        X1 = [X1, vadj{3}];
                        vLag{2*k+1} = myLagmatrix(vadj{2*k+1}, Pw12c);
                        X1 = [X1, vLag{2*k+1}];
                    end
                end

                vLag{1+2*wlagMax+1} = myLagmatrix(vadj{1+2*wlagMax+1}, Pw12c);
                X1 = [X1, vadj{4}, vLag{4}];
                vLag{1+2*wlagMax+2} = myLagmatrix(vadj{1+2*wlagMax+2}, Pw12c);
                X1 = [X1, vadj{5}, vLag{5}];
                ...
                if wlagMaxReg > 0
                    for k = 1:1
                        xcLag{2*k} = myLagmatrix(xadj{2*k}, Pw13c);
                        X1 = [X1, xadj{2}, xcLag{2}];
                        xcLag{2*k+1} = myLagmatrix(xadj{2*k+1}, Pw13c);
                        X1 = [X1, xadj{3}, xcLag{3}];
                    end
                end

                xcLag{1+2*wlagMaxReg+1} = myLagmatrix(xadj{1+2*wlagMaxReg+1}, Pw13c);
                X1 = [X1, xadj{4}, xcLag{4}];
                xcLag{1+2*wlagMaxReg+2} = myLagmatrix(xadj{1+2*wlagMaxReg+2}, Pw13c);
...
crPvec1=regPvecMat(j1,:);
crPvec2=regPvecMat(j2,:);
crPvec3=j3;
...
for w = (1+w00):(wmax-w00)    
    ...
    [yadj, xadj, vadj, zadj] = parseSpatiallyOrganizedTS(w, Mapsy, Mapsx, Mapsz, Mapsv);
...
repmat([ones(1,Pw11+1),zeros(1,tlagMax-Pw11)],1,4)];
tmpIndvec3 = [ones(1,Pw02+1), zeros(1,tlagMax-Pw02), ...
repmat([ones(1,Pw12+1),zeros(1,tlagMax-Pw12)],1,4)];
tmpIndvec4= repmat([ones(1,Pw13+1),zeros(1,tlagMax-Pw13)],1,4);  
...
vLag = cell(2*wlagMax+1+2, 1);
    ...
    %include V_t(w,l) and all adjacent signals at t
    X1 = [X1, vadj{1}];
