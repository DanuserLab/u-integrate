function [arMdlAvBICmap, arMdlAvBICcurve, reducedMdlAvBICmap, reducedMdlAvBICcurve, ...
    ...
    Pw1, Qw0, modelIndVarsMat, modelCoefMat, winFitsMat, resiMat, yhatFMat, yhatRMat] = ...
    SPAR_2ch_mdlSlctn_iGC_3steps(Mapsx, Mapsy,  ...
    tlagMax, wlagMax, tlagMaxReg, wlagMaxReg, regPvecMat, ICtype)
% SPAR_2ch_mdlSlctn_iGC_3steps() IMPLEMENT the regression fitting and model
...
parfor w = (1+w00):(wmax-w00)    
    ...
    [yadj, ~] = parseSpatiallyOrganizedTS(w, Mapsy, Mapsx);
...
parfor w = (1+w00):(wmax-w00)    
    %disp(w)
    fprintf(1, '%g ', w); if (mod(w,50)==0); fprintf('\n'); end
    [yadj, xadj] = parseSpatiallyOrganizedTS(w, Mapsy, Mapsx);
...
modelIndVarsMat = nan(wmax, (5)*tlagMax +  (5)*(tlagMaxReg+1));
...
modelIndVarsFixed = [tmpIndvec,  tmpIndvec3, tmpIndvecReg];
...
function [yadj, xadj] = parseSpatiallyOrganizedTS(w, Mapsy, Mapsx)
